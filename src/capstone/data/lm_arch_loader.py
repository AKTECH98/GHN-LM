...
from capstone.ghn.graph import Graph, GraphBatch
...
def create_model_from_config(model_type: str, config: Dict, device: str = 'cpu'):
    ...
    model_config = ModelConfig(
        ...
        vocab_size=config_copy.pop('vocab_size', 50257),
        d_model=config_copy['d_model'],
        n_layer=config_copy['n_layer'],
        n_head=config_copy['n_head'],
        d_ff=config_copy['d_ff'],
        max_seq_len=config_copy.get('max_seq_len', config_copy.pop('max_len', 512)),
        p_drop=config_copy.get('p_drop', 0.1),
    )
    from capstone.lm.create_model import create_model
    return create_model(model_config, model_config.vocab_size).to(device)
