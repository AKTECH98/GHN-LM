#!/bin/bash
# Script to run configs 11-20 through different initialization methods (excluding GHN)
# This will submit SLURM jobs for each config (11-20) with each init method
...
# Find benchmark config files 11-20
CONFIG_FILES=($(ls -1 "$CONFIG_DIR"/benchmark_1[1-9]_*.yaml "$CONFIG_DIR"/benchmark_20_*.yaml 2>/dev/null | sort))

if [ ${#CONFIG_FILES[@]} -eq 0 ]; then
    echo "❌ Error: No benchmark config files found in $CONFIG_DIR for configs 11-20"
    exit 1
fi

echo "=========================================="
echo "Running Configs 11-20 with All Init Methods"
echo "=========================================="
echo "Found ${#CONFIG_FILES[@]} config files (11-20)"
...
# e.g., "LM/configs/benchmark_11_tiny_transformers.yaml" -> "11_tiny_transformers"
get_config_name() {
    local config_file="$1"
...
# e.g., "11_tiny_transformers" + "default" -> "Benchmark_11_tiny_transformers"
# e.g., "11_tiny_transformers" + "he" -> "BenchmarkHEInit_11_tiny_transformers"
# e.g., "11_tiny_transformers" + "xavier" -> "BenchmarkXavier_11_tiny_transformers"
get_experiment_name() {
    local config_name="$1"
    local init_method="$2"
...
# Loop through all config files (11-20)
